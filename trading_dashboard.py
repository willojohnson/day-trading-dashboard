...
bullish_strategies = [
    ...
    "Golden Cross", "Trend + MACD Bullish", "Golden Cross + Volume", # Existing confirmations
    "Bollinger Bullish Breakout", # Existing confirmation
    "Stochastic Bullish Crossover (Oversold)", # NEW
    "OBV Bullish Trend Confirmation" # NEW
]
bearish_strategies = [
    "MACD Bearish Crossover", "RSI Overbought", "Death Cross",
    "Death Cross + RSI Bearish", "Death Cross + Volume", # Existing confirmations
    "Bollinger Bearish Breakout", # Existing confirmation
    "Stochastic Bearish Crossover (Overbought)", # NEW
    "OBV Bearish Trend Confirmation" # NEW
]
...
st.sidebar.markdown("---")
st.sidebar.markdown("**Stochastic Bullish Crossover (Oversold)**: %K crosses above %D AND both are below 30") # NEW
st.sidebar.markdown("**Stochastic Bearish Crossover (Overbought)**: %K crosses below %D AND both are above 70") # NEW
st.sidebar.markdown("**OBV Bullish Trend Confirmation**: Current Close > Previous Close AND Current OBV > Previous OBV") # NEW
st.sidebar.markdown("**OBV Bearish Trend Confirmation**: Current Close < Previous Close AND Current OBV < Previous OBV") # NEW
...
for ticker in TICKERS:
    ...
    try:
        ...
        # --- Debugging: Print last few rows of DataFrame ---
        # print(f"\n--- DEBUG: Data for {ticker} ---")
        # print(df.tail(5)) # Print last 5 rows to inspect data and NaNs
        ...
        # Also need at least 20 periods for Bollinger Bands, 14 for Stochastic
        if len(df) < 200:
            st.info(f"ℹ️ Not enough historical data for {ticker} ({company}) for 200-period MA, Bollinger Bands or Stochastic calculation (need 200 bars). Skipping strategies.")
        ...
        df['%D'] = df['%K'].rolling(window=3).mean() # 3-period SMA of %K

        # NEW: On-Balance Volume (OBV)
        # Initialize OBV with 0 for the first value, then apply logic
        df['OBV'] = 0
        ...
        # NEW Stochastic & OBV scalars
        k_1, d_1, k_2, d_2, obv_1, obv_2 = [None] * 6
        ...
        try:
            ...
            if not df.empty and len(df) >= 1:
                ...
                k_1 = get_scalar_value(df['%K'].iloc[-1]) # NEW
                d_1 = get_scalar_value(df['%D'].iloc[-1]) # NEW
                obv_1 = get_scalar_value(df['OBV'].iloc[-1]) # NEW
            ...
            if not df.empty and len(df) >= 2:
                ...
                k_2 = get_scalar_value(df['%K'].iloc[-2]) # NEW
                d_2 = get_scalar_value(df['%D'].iloc[-2]) # NEW
                obv_2 = get_scalar_value(df['OBV'].iloc[-2]) # NEW
        ...
        required_scalars = [ma20_1, ma50_1, ma200_1, rsi_1, macd_1, macd_signal_1,
                            ...
                            k_1, d_1, k_2, d_2, obv_1, obv_2] # NEW required scalars
        ...
        if any(s is None for s in required_scalars):
            ...
            for s_name, s_value in zip(['ma20_1', 'ma50_1', 'ma200_1', 'rsi_1', 'macd_1', 'macd_signal_1',
                                        ...
                                        'close_1', 'close_2', 'upper_bb_1', 'upper_bb_2', 'lower_bb_1', 'lower_bb_2',
                                        'k_1', 'd_1', 'k_2', 'd_2', 'obv_1', 'obv_2'], required_scalars):
                if s_value is None:
        ...
        # NEW: Stochastic Crossover Conditions
        stoch_bullish_crossover_oversold = (k_2 < d_2 and k_1 > d_1 and k_1 < 30 and d_1 < 30)
        stoch_bearish_crossover_overbought = (k_2 > d_2 and k_1 < d_1 and k_1 > 70 and d_1 > 70)

        # NEW: OBV Trend Confirmation
        ...
        # NEW Bullish Strategies
        if "Stochastic Bullish Crossover (Oversold)" in selected_bullish and stoch_bullish_crossover_oversold:
            signals.append((ticker, "bullish", f"⬆️ Bullish - Stochastic Crossover (Oversold) — {company}"))
            heatmap_row["Stochastic Bullish Crossover (Oversold)"] = 1
        ...
        # NEW Bearish Strategies
        if "Stochastic Bearish Crossover (Overbought)" in selected_bearish and stoch_bearish_crossover_overbought:
            signals.append((ticker, "bearish", f"⬇️ Bearish - Stochastic Crossover (Overbought) — {company}"))
            heatmap_row["Stochastic Bearish Crossover (Overbought)"] = 1
